## =============================================================================
## The parallel processing code for quarter-split wood moisture diffusion
## =============================================================================
...
paramain<-function(r){
    ...
    phi.grid    <- setup.grid.1D(x.up = 0, x.down = pi/2, N = N)
...
quarteroutput<-foreach(r=rsectional, 
                .combine = abindcombine, .packages='ReacTran')  %dopar%  
    paramain(r)

stopImplicitCluster()

save(quarteroutput,file=paste('quarteroutput',timestepprecision,sep=' '))
# Reshuffled to time, radius, phi and z (Phi and Z? A different N would make things a lot easier)
quarteroutput<-aperm(quarteroutput,c(1,4,2,3))
...
quarteroutput<-quarteroutput*bigr
quarteroutput<-array(quarteroutput,dim=c(timestepprecision,N*N*N))
summa<-matrix(1/N/N,nrow=N*N*N)
quarteraveragemoisture<-quarteroutput%*%summa
qplot(x=seq(1/timestepprecision,maxtime,length.out = timestepprecision),y=quarteraveragemoisture)
save(quarteraveragemoisture,file='quarteraveragemoisture')
